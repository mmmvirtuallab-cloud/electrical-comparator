...
const ElectricalComparator = () => {
  ...
  // ***CHANGE 1: Initialize state to null***
  // No workpiece is selected on load.
  const [workpieceType, setWorkpieceType] = useState(null); // 'flat1', 'flat2', 'flat3', or null
  ...
  useEffect(() => {
    setIsHeadLocked(false);
    setHeadY(0);
    setWorkpiecePosition({ x: 0, y: 0 });
    setCurrentPage(1);
  }, [workpieceType]);
  ...
  // oldMaxHeadY = 160 (from original logic), which was the correct value for the plunger *body* to touch.
  // The new tip is 10px long (from CSS), so we subtract that.
  const plungerTipHeight = 10;
  const maxHeadY =
    160 - plungerTipHeight - workpieceVisualHeight - workpiecePosition.y;
  ...
  const workpieceStyle = {
    ...
    borderRadius: "4px",
    // Changed 'backgroundColor' to 'background' to override the CSS file's 'background: linear-gradient'
    background:
      workpieceType === "flat1"
        ? "#2ecc71" // Standard WP - Green
        : workpieceType === "flat2"
        ? "#3498db" // WP 1 - Blue
        : "#e74c3c", // WP 2 - Red
  };
  ...
  const handleAddReading = () => {
    ...
    const measuredHeight = (
      standardHeight - Number.parseFloat(displayValue)
    ).toFixed(3);
    const newReading = {
      id: Date.now(),
      standard: standardHeight.toFixed(3),
      measured: measuredHeight,
      deviation: Number.parseFloat(displayValue).toFixed(3),
    };
      ...
      <div className="main-content-area">
        ...
        <div className="comparator-container">
          <div className="comparator-rig">
            ...
            {/* ***CHANGE 2: Conditionally render the workpiece*** */}
            {/* This Draggable component will only render if workpieceType is not null */}
            {workpieceType && (
              <Draggable
                nodeRef={tableRef}
                axis="x"
              ...
              >
                <div ref={tableRef} className="table-workpiece-assembly">
                  <div className="workpiece" style={workpieceStyle}>
                    {/* Corrected labels to match selectors */}
                    <span>
                      {workpieceType === "flat1"
                        ? "STD WP"
                        : workpieceType === "flat2"
            ...
            <div className="button-gro">
              <div className="arrow-controls">
                ...
                <div className="arrow-row">
                  ...
                  <button
                    ref={workpieceRightRef}
                    className="move-btn arrow-right"
                    onClick={moveRight}
                  >
                    <FaArrowRight />
                  </button>
      ...
      {currentNotification && currentNotification.ref.current && (
        <div
          ...
          style={{
            position: "fixed",
            top:
              currentNotification.ref.current.getBoundingClientRect().top - 40,
            left: currentNotification.ref.current.getBoundingClientRect().left,
          }}
        >
              ...
              <table>
                <thead>
                  <tr>
                    <th>Standard (mm)</th>
                    <th>Measured (mm)</th>
                    <th>Deviation Limit (0-0.500)(mm)</th>
                    <th>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {currentReadings.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td
                        className={
                          Number.parseFloat(r.deviation) > 0
                            ? "positive"
                            : Number.parseFloat(r.deviation) < 0
                            ? "negative"
                            : ""
                      ...
                      <td>
                        {Number.parseFloat(r.deviation) > 0.5 ? (
